...
;; =============================================================================
;; Typed Interpreter: test-support.rkt
;; =============================================================================
...
; test-raises-type-error?
; Tests if the given thunk-body raises an error of type exn:fail:type, defined in err-support.rkt
(define-syntax-rule (test-raises-type-error? test-case-name thunk-body)
  (*test-exn test-case-name
             (lambda (exn) (exn:fail:type? exn))
             (thunk thunk-body)))

; test-raises-interp-error
; Tests if the given thunk-body raises an error of type exn:fail:interp, defined in err-support.rkt
(define-syntax-rule (test-raises-interp-error? test-case-name thunk-body)
  (*test-exn test-case-name
             (lambda (exn) (exn:fail:interp? exn))
             (thunk thunk-body)))

;; SPECIFIC TESTING UTILITIES =======================================

; Import of support code.
(require "support.rkt" "err-support.rkt" (only-in plait s-exp))
